...
'''
서비스 계정 키를 로드하여 Vision API에 인증하는 함수
'''

class OCR(Resource):

    def __init__(self):
        self.credentials_path = 'api/absolute-realm-391102-64dbe04d4fc4.json'
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('base64')
        args=parser.parse_args()
        print(args['base64'])
        print(type(args['base64']))
        texts= self.detect_labels(args['base64'])
        print(texts)
        # return make_response({'texts':''.join(texts)})
        return make_response(''.join(texts))

    def authenticate_service_account(self):
        '''
        서비스 계정 키를 로드하여 구글 vision API에 인증하는 함수
        credentials_path : private key가 있는 json파일의 경로
        return : 인증정보 객체
        '''
    ...
    def detect_labels(self,image_base64):
        ...
        객체탐지결과를 받는 함수
        image_base64 : 이미지의 BASE64 인코딩 문자열
        ...
        # 이미지 데이터 디코딩 (이미지가 아닌 바이너리 데이터가 반환 - BYTES 타입)
