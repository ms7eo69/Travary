...
public class FileUtils {
	...
	/**
	 * 다중 파일 업로드용 메소드로 파일을 하나라도 업로드시는 파일명이 담긴 StringBuffer 반환
	 * @param parts					Collection<Part>으로 업로드된 파라미터 정보를 갖고있는 Parts객체 컬렉션
	 * @param saveDirectory	서버의 파일이 저장된 물리적 경로
	 * @return 파일명을 저장한 StringBuffer객체 
	 */
	//업로드 로직
	public static StringBuffer upload(Collection<Part> parts,String saveDirectory) {
		...
		try {			
			for(Part part:parts) {
				if(part.getContentType() !=null) {	//input type="file"인 경우에만 not null		
					String systemFileName=FileUtils.getNewFileName(saveDirectory, part.getSubmittedFileName());
					//파일명을 스트링버퍼에 저장
					fileNames.append(systemFileName+",");//구분자 , (내맘)
					//파일 업로드
