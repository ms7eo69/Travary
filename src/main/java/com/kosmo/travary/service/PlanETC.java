...
public class PlanETC {
	...
	@GetMapping("/plan/geo.do")
	@ResponseBody
	public Map geolocation(@RequestParam String  query) {
		String url = "https://naveropenapi.apigw.ntruss.com/map-geocode/v2/geocode?query="+query;
		Map result = new RestTemplate().exchange(url, HttpMethod.GET, new HttpEntity<>(setNaverHeaders()), Map.class).getBody();
		return result;	
	}
}
//	
